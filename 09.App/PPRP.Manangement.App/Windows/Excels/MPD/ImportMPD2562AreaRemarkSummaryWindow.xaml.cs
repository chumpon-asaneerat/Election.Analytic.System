...
{
    /// <summary>
    /// Interaction logic for ImportMPD2562AreaRemarkSummaryWindow.xaml
    /// </summary>
    public partial class ImportMPD2562AreaRemarkSummaryWindow : Window
    {
        #region Constructor
        ...
        /// Constructor.
        /// </summary>
        public ImportMPD2562AreaRemarkSummaryWindow()
        {
            InitializeComponent();
        }
        ...
        public class XlsMPD2562PollingUnitSummary : MPD2562PollingUnitSummary
        {
            public static List<XlsMPD2562PollingUnitSummary> LoadWorksheetTable(NExcelImport import,
                string sheetName, List<NExcelMapProperty> mapProperties)
            {
                MethodBase med = MethodBase.GetCurrentMethod();

                var results = new List<XlsMPD2562PollingUnitSummary>();
                ...
                using (var package = new ExcelPackage(import.FileName))
                {
                    try
                    {
                        ...
                        if (null != sheet)
                        {
                            ...
                            {
                                var inst = new XlsMPD2562PollingUnitSummary();
                                ...
                                foreach (var key in columns.Keys)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        DynamicAccess<XlsMPD2562PollingUnitSummary>.Set(inst, key, oVal);
                                    }
                                    catch (Exception ex)
                                    {
        ...
        private List<XlsMPD2562PollingUnitSummary> items = null;
        ...
        private void Import_OnSampleDataChanged(object sender, EventArgs e)
        {
            if (null != wsMap && null != wsMap.ImportModel)
            {
                ...
                items = XlsMPD2562PollingUnitSummary.LoadWorksheetTable(import, model.Worksheet.SheetName, model.Maps);
                if (null != items)
                {
        ...
        private void ChooseExcelFile()
        {
            if (import.ShowDialog(PPRPApp.Windows.MainWindow))
            {
                ...
                var mapProperties = new string[][]
                {
                    ...
                    new string[] { "AreaRemark", "ข้อมูลพื้นที่" }
                };
        ...
        private void Imports()
        {
            if (null != items && items.Count > 0)
            {
                ...
                {
                    MPD2562PollingUnitSummary.ImportAreaRemark(item);
                    prog.Increment();
                }
