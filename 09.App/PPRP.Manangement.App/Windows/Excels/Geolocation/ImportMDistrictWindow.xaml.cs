...
namespace PPRP.Windows
{
    ...
    public partial class ImportMDistrictWindow : Window
    {
        ...
        public class XlsMDistrict : MDistrict
        {
            public static List<XlsMDistrict> LoadWorksheetTable(NExcelImport import,
                string sheetName, List<NExcelMapProperty> mapProperties)
            {
                MethodBase med = MethodBase.GetCurrentMethod();

                var results = new List<XlsMDistrict>();
                            ...
                            for (int row = 2; row <= rowCount; row++)
                            {
                                ...
                                foreach (var key in columns.Keys)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        DynamicAccess<XlsMDistrict>.Set(inst, key, oVal);
                                    }
                                    catch (Exception ex)
                                    {
        ...
        private List<XlsMDistrict> items = null;
        ...
        private void Import_OnSampleDataChanged(object sender, EventArgs e)
        {
            if (null != wsMap && null != wsMap.ImportModel)
            {
                ...
                items = XlsMDistrict.LoadWorksheetTable(import, model.Worksheet.SheetName, model.Maps);
                if (null != items)
                {
