...
namespace PPRP.Windows
{
    ...
    public partial class ImportMProvinceWindow : Window
    {
        ...
        public class XlsMProvince : MProvince
        {
            public static List<XlsMProvince> LoadWorksheetTable(NExcelImport import,
                string sheetName, List<NExcelMapProperty> mapProperties)
            {
                MethodBase med = MethodBase.GetCurrentMethod();

                var results = new List<XlsMProvince>();
                            ...
                            for (int row = 2; row <= rowCount; row++)
                            {
                                ...
                                foreach (var key in columns.Keys)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        DynamicAccess<XlsMProvince>.Set(inst, key, oVal);
                                    }
                                    catch (Exception ex)
                                    {
        ...
        private List<XlsMProvince> items = null;
